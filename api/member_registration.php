...
if ($fullName === '' || !$email || $phone === '' || $schoolName === '') {
    ...
    redirect('public/?page=anggota');
}

$stmt = db()->prepare('INSERT INTO member_registrations (full_name, identity_number, email, phone, school_name, job_title, district, address, reason) VALUES (?,?,?,?,?,?,?,?,?)');
$stmt->execute([
    $fullName,
    $identityNumber ?: null,
    $email,
