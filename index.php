...
function render_latest_posts_sidebar(?string $currentSlug = null): void
{
    ...
    <aside class="latest-posts-sidebar">
        ...
        <?php foreach ($latestPosts as $latestPost): ?>
            ...
            $shown++;
            ?>
            <a class="latest-post-item" href="<?= e(url('?page=detail&slug=' . $latestPost['slug'])) ?>">
...
if ($page === 'profil') {
    ...
    <section class="section-band section-soft">
        <div class="container">
            <div class="d-flex flex-wrap gap-2 mb-4">
                <a class="btn btn-sm btn-outline-primary" href="<?= e(url('?page=berita')) ?>">Semua</a>
                <?php foreach ($categories as $cat): ?><a class="btn btn-sm btn-outline-primary" href="<?= e(url('?page=berita&kategori=' . $cat['slug'])) ?>"><?= e($cat['name']) ?></a><?php endforeach; ?>
            </div>
                ...
                <div class="col-lg-8">
                        ...
                        <?php foreach ($posts as $post): ?>
                            <div class="col-md-6">
                                <article class="card card-official h-100">
                                    <?php if (has_custom_post_image($post['image'])): ?><a class="post-image-link" href="<?= e(url('?page=detail&slug=' . $post['slug'])) ?>"><img loading="lazy" src="<?= e(media_url($post['image'])) ?>" alt="<?= e($post['title']) ?>"></a><?php endif; ?>
                                    <div class="card-body">
                                        ...
                                        <h5><?= e($post['title']) ?></h5>
                                        <p><?= e($post['excerpt']) ?></p>
                                        <a class="fw-semibold" href="<?= e(url('?page=detail&slug=' . $post['slug'])) ?>">Baca selengkapnya</a>
                                    </div>
                                </article>
                            </div>
                        <?php endforeach; ?>
                    ...
                    <nav class="mt-4"><ul class="pagination">
                        <?php for ($i = 1; $i <= max(1, ceil($total / $limit)); $i++): ?>
                            <li class="page-item <?= $i === $currentPage ? 'active' : '' ?>"><a class="page-link" href="<?= e(url('?page=berita' . ($category !== '' ? '&kategori=' . urlencode($category) : '') . '&p=' . $i)) ?>"><?= $i ?></a></li>
                        <?php endfor; ?>
                    </ul></nav>
                </div>
                <div class="col-lg-4">
    ...
    $schoolPageUrl = function (int $targetPage) use ($district, $perPage): string {
        ...
        return url('?' . http_build_query($query)) . '#schoolDataSection';
    };
    ?>
    ...
    <section class="section-band" id="schoolDataSection">
        <div class="container">
            <form class="row g-3 mb-4" method="get" action="<?= e(url('')) ?>#schoolDataSection" id="schoolFilterForm">
                            ...
                            <td>
                                <div class="member-profile-cell">
                                    <?php if (!empty($member['photo'])): ?>
                                        <img class="member-avatar" src="<?= e(media_url($member['photo'])) ?>" alt="Foto <?= e($member['full_name']) ?>">
                                    <?php else: ?>
                                        <span class="member-avatar member-avatar-placeholder"><i class="fa-solid fa-user"></i></span>
                                    <?php endif; ?>
                                    <div><strong><?= e($member['full_name']) ?></strong><br><small><?= e($member['job_title'] ?: 'Anggota PGRI') ?></small></div>
                                </div>
                            </td>
                ...
                <form action="<?= e(url('api/member_registration.php')) ?>" method="post" enctype="multipart/form-data">
                    <div class="modal-body">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-5"><label class="form-label">Foto Anggota</label><input class="form-control" type="file" name="photo" accept="image/jpeg,image/png,image/webp,image/*" data-max-size="5242880"><small class="text-muted">Opsional. Format JPG, PNG, atau WEBP maksimal 5MB.</small></div>
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-8">
                    ...
                    <a href="<?= e(url('?page=profil')) ?>" class="btn btn-light btn-lg me-2">Lihat Profil</a>
                    <a href="<?= e(url('?page=kontak')) ?>" class="btn btn-outline-light btn-lg">Keluhan</a>
                </div>
            </div>
        </div>
    </header>
    <section class="section-band">
        <div class="container">
            ...
            <div class="row g-4 align-items-start mt-2">
                <div class="col-lg-8">
                    <div class="row g-4">
                        <?php foreach ($posts as $post): ?><div class="col-md-6 col-xl-4"><article class="card card-official h-100"><?php if (has_custom_post_image($post['image'])): ?><a class="post-image-link" href="<?= e(url('?page=detail&slug=' . $post['slug'])) ?>"><img loading="lazy" src="<?= e(media_url($post['image'])) ?>" alt="<?= e($post['title']) ?>"></a><?php endif; ?><div class="card-body"><span class="badge text-bg-danger mb-2"><?= e($post['category_name']) ?></span><?php if (!empty($post['video'])): ?> <span class="badge text-bg-primary mb-2">Video</span><?php endif; ?><h5><?= e($post['title']) ?></h5><p><?= e($post['excerpt']) ?></p><a class="fw-semibold" href="<?= e(url('?page=detail&slug=' . $post['slug'])) ?>">Baca selengkapnya</a></div></article></div><?php endforeach; ?>
                    </div>
                </div>
                <div class="col-lg-4">
                    <?php render_latest_posts_sidebar(); ?>
    ...
    <section class="section-band section-soft">
        <div class="container">
            <div class="row g-5 align-items-center">
                ...
                <div class="col-lg-7"><h2 class="section-title">Sambutan Ketua PGRI</h2><p class="fs-5">Selamat datang di website resmi PGRI Kotamobagu. Media ini menjadi ruang informasi, layanan, dan kolaborasi bagi seluruh insan pendidikan. Kehadiran website ini kami harapkan mampu mempercepat penyampaian informasi organisasi, memperkuat komunikasi antaranggota, serta menjadi sarana publikasi kegiatan PGRI yang transparan dan mudah diakses masyarakat.</p><p class="fs-5">PGRI Kotamobagu terus berkomitmen mendukung peningkatan kompetensi, perlindungan profesi, dan kesejahteraan guru. Melalui kerja sama yang solid antara pengurus, anggota, sekolah, pemerintah daerah, dan seluruh pemangku kepentingan pendidikan, kami percaya kualitas pendidikan di Kota Kotamobagu dapat terus tumbuh secara berkelanjutan.</p><p class="fs-5">Mari bergerak bersama memperkuat profesionalisme guru, membangun budaya belajar yang adaptif, dan menghadirkan layanan pendidikan yang bermutu bagi generasi masa depan.</p><p class="fw-semibold mb-0"><?= e($chairpersonName ?: $chairpersonPosition) ?></p><p class="text-muted mb-0"><?= e($chairpersonPosition) ?></p></div>
            </div>
        </div>
    </section>
    <section class="section-band">
