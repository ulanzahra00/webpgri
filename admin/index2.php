...
try {
    ...
    if ($module === 'dashboard') {
        ...
        $output = '<div class="row g-4"><div class="col-12"><div class="card card-official"><div class="card-body"><h5>Selamat datang</h5><p class="mb-0">Gunakan menu admin untuk mengelola konten website resmi PGRI Kotamobagu.</p></div></div></div></div>';
        ...
        <tr><td><div class="member-profile-cell"><?php if (!empty($row['photo'])): ?><img class="member-avatar" src="<?= e(media_url($row['photo'])) ?>" alt="Foto"><?php else: ?><span class="member-avatar member-avatar-placeholder"><i class="fa-solid fa-user"></i></span><?php endif; ?><div><strong><?= e($row['full_name']) ?></strong><br><small><?= e($row['identity_number'] ?: '-') ?></small><br><small><?= date('d M Y H:i', strtotime($row['created_at'])) ?></small></div></div></td>
        ...
        <td><form method="post" action="<?= e(url('admin/actions.php')) ?>"><input type="hidden" name="csrf_token" value="<?= csrf_token() ?>"><input type="hidden" name="module" value="member_registrations"><input type="hidden" name="id" value="<?= e($row['id']) ?>"><select class="form-select form-select-sm mb-2" name="status"><?php foreach ($statusLabels as $value => $label): ?><option value="<?= e($value) ?>" <?= $row['status'] === $value ? 'selected' : '' ?>><?= e($label) ?></option><?php endforeach; ?></select><textarea class="form-control form-control-sm mb-2" name="notes" rows="2" placeholder="Catatan"><?= e($row['notes']) ?></textarea><button class="btn btn-sm btn-pgri">Simpan</button></form></td>
        <td><a data-confirm="Hapus?" class="btn btn-sm btn-outline-danger" href="<?= e(url('admin/actions.php?module=member_registrations&action=delete&id=' . (int)$row['id'] . '&csrf_token=' . csrf_token())) ?>"><i class="fa-solid fa-trash"></i></a></td></tr>
        <?php endforeach; ?>
        </tbody></table></div></div>
        <?php
        $output = ob_get_clean();
        ...
        if ($action === 'form') {
            ...
            if ($id) { $stmt = db()->prepare('SELECT * FROM posts WHERE id = ?'); $stmt->execute([$id]); $post = $stmt->fetch() ?: $post; }
            ...
            <div class="col-md-6"><label class="form-label">Gambar</label><input class="form-control" type="file" name="image" accept="image/*"><small>Kosongkan jika tidak mengganti.</small></div>
            <div class="col-md-6"><label class="form-label">Video MP4</label><input class="form-control" type="file" name="video" accept="video/mp4,.mp4"><small>Opsional.</small></div>
            ...
            </div><button class="btn btn-pgri mt-4">Simpan</button></form><?php
        } else {
            ...
            <div class="card card-official"><div class="table-responsive"><table class="table mb-0"><thead><tr><th>Judul</th><th>Kategori</th><th>Status</th><th>Aksi</th></tr></thead><tbody>
            <?php foreach ($rows as $row): ?><tr><td><?= e($row['title']) ?></td><td><?= e($row['category_name']) ?></td><td><?= e($row['status']) ?></td><td><?= admin_table_actions('posts', (int)$row['id']) ?></td></tr><?php endforeach; ?>
        ...
        if ($action === 'form') {
            ...
            <div class="row g-3"><div class="col-md-6"><label class="form-label">Tanggal</label><input class="form-control" type="date" name="event_date" value="<?= e($row['event_date']) ?>"></div>
            <div class="col-md-6"><label class="form-label">Gambar</label><input class="form-control" type="file" name="image" accept="image/*" <?= $id ? '' : 'required' ?>></div></div>
            <button class="btn btn-pgri mt-4">Simpan</button></form><?php
        } else {
            ...
            foreach ($rows as $row) { echo '<div class="col-md-4"><div class="card card-official"><img src="' . e(media_url($row['image'])) . '" alt=""><div class="card-body"><h5>' . e($row['title']) . '</h5>' . admin_table_actions('galleries', (int)$row['id']) . '</div></div></div>'; }
            echo '</div>';
        }
        $output = ob_get_clean();
        ...
        if ($action === 'form') {
            ...
            <?php include __DIR__ . '/partials/school_modals.php'; ?>
        ...
        if ($action === 'form') {
            ...
            <div class="col-md-4"><label class="form-label">Kategori</label><input class="form-control" name="category" value="<?= e($row['category']) ?>" required></div>
            ...
            <div class="col-md-6"><label class="form-label">Dokumen</label><input class="form-control" type="file" name="document" accept=".pdf,.xls,.xlsx"></div>
            ...
            if ($filterMonth === 'annual') $filterMonth = 0;
            elseif ($filterMonth !== '') {
                $filterMonth = (int)$filterMonth;
                if ($filterMonth < 1 || $filterMonth > 12) $filterMonth = '';
            }
            $where = '';
            $params = [];
            if ($filterMonth !== '') { $where = 'WHERE period_month = ?'; $params[] = $filterMonth; }
            ...
            ?><div class="d-flex flex-wrap gap-2 justify-content-between mb-3"><h4>Keuangan</h4><a class="btn btn-pgri" href="<?= e(url('admin/?module=finance&action=form')) ?>">Tambah</a></div>
            <div class="row g-3 mb-3"><div class="col-md-4"><div class="stat-tile p-3"><small>Pemasukan</small><h5 class="text-success mb-0"><?= rupiah((float)$totals['income_total']) ?></h5></div></div>
            <div class="col-md-4"><div class="stat-tile p-3"><small>Pengeluaran</small><h5 class="text-danger mb-0"><?= rupiah((float)$totals['expense_total']) ?></h5></div></div>
            <div class="col-md-4"><div class="stat-tile p-3"><small>Saldo</small><h5 class="text-primary mb-0"><?= rupiah((float)$totals['balance_total']) ?></h5></div></div></div>
            <div class="card card-official"><div class="table-responsive"><table class="table mb-0"><thead><tr><th>Periode</th><th>Judul</th><th>Pemasukan</th><th>Pengeluaran</th><th>Aksi</th></tr></thead><tbody>
            <?php foreach ($rows as $row): ?><tr><td><?= e($periodOptions[(int)$row['period_month']] ?? '-') ?> <?= e($row['period_year']) ?></td><td><?= e($row['title']) ?></td><td class="text-success"><?= rupiah((float)$row['income']) ?></td><td class="text-danger"><?= rupiah((float)$row['expense']) ?></td><td><?= admin_table_actions('finance', (int)$row['id']) ?></td></tr><?php endforeach; ?>
            </tbody></table></div></div><?php
        }
        $output = ob_get_clean();
        ...
        if ($action === 'form') {
            ...
            <div class="row g-3"><div class="col-md-6"><label class="form-label">Nama</label><input class="form-control" name="name" value="<?= e($row['name']) ?>" required></div>
            <div class="col-md-6"><label class="form-label">Jabatan</label><input class="form-control" name="position" value="<?= e($row['position']) ?>" required></div>
            <div class="col-md-4"><label class="form-label">Urutan</label><input class="form-control" type="number" name="sort_order" value="<?= e($row['sort_order']) ?>"></div>
            <div class="col-md-8"><label class="form-label">Foto</label><input class="form-control" type="file" name="photo" accept="image/*"></div>
            <div class="col-12"><label class="form-label">Bio</label><textarea class="form-control" name="bio"><?= e($row['bio']) ?></textarea></div></div>
            <button class="btn btn-pgri mt-4">Simpan</button></form><?php
        } else {
        ...
        if ($action === 'form') {
            ...
            <div class="row g-3"><div class="col-md-6"><label>Nama</label><input class="form-control" name="name" value="<?= e($row['name']) ?>" required></div>
            <div class="col-md-6"><label>Email</label><input class="form-control" type="email" name="email" value="<?= e($row['email']) ?>" required></div>
            <div class="col-md-6"><label>Password <?= $id ? 'Baru' : '' ?></label><input class="form-control" type="password" name="password" <?= $id ? '' : 'required' ?>></div>
            <div class="col-md-6"><label>Role</label><select class="form-select" name="role"><option value="admin">Admin</option><option value="superadmin" <?= $row['role'] === 'superadmin' ? 'selected' : '' ?>>Superadmin</option></select></div></div>
            <button class="btn btn-pgri mt-4">Simpan</button></form><?php
        } else {
            $rows = db()->query('SELECT id,name,email,role FROM users ORDER BY id DESC')->fetchAll();
            echo '<div class="d-flex justify-content-between mb-3"><h4>Kelola User</h4><a class="btn btn-pgri" href="' . e(url('admin/?module=users&action=form')) . '">Tambah User</a></div><div class="card card-official"><table class="table mb-0"><thead><tr><th>Nama</th><th>Email</th><th>Role</th><th>Aksi</th></tr></thead><tbody>';
        ...
        <?php foreach ($rows as $row): ?>
            ...
            <div class="mb-3"><label class="form-label">Banner Header</label><?php if ($row['setting_value']): ?><img class="img-fluid rounded mb-2" src="<?= e(media_url($row['setting_value'])) ?>" alt="Banner"><?php endif; ?><input type="hidden" name="settings[hero_banner]" value="<?= e($row['setting_value']) ?>"><input class="form-control" type="file" name="hero_banner" accept="image/*"><small>Kosongkan jika tidak mengganti.</small></div>
            <?php elseif ($row['setting_key'] === 'site_logo'): ?>
            <div class="mb-3"><label class="form-label">Logo Website</label><?php if ($row['setting_value']): ?><div class="mb-2"><span class="logo-mark"><img src="<?= e(media_url($row['setting_value'])) ?>" alt="Logo"></span></div><?php endif; ?><input type="hidden" name="settings[site_logo]" value="<?= e($row['setting_value']) ?>"><input class="form-control" type="file" name="site_logo" accept="image/*"><small>Kosongkan jika tidak mengganti.</small></div>
            <?php elseif ($row['setting_key'] === 'whatsapp_number'): ?>
            <div class="mb-3"><label class="form-label">WhatsApp</label><input class="form-control" name="settings[whatsapp_number]" value="<?= e($row['setting_value']) ?>" placeholder="6281234567890"><small>Format internasional tanpa +.</small></div>
            <?php else: ?>
            ...
            <?php endif; ?>
        <?php endforeach; ?>
        <button class="btn btn-pgri">Simpan Pengaturan</button></form><?php
        $output = ob_get_clean();
    } else {
        $output = '<div class="alert alert-warning">Module tidak dikenal: ' . e($module) . '</div>';
    }

    echo $output;
    require_once __DIR__ . '/../includes/admin_footer.php';
} catch (Throwable $e) {
    echo '<div style="color:red;padding:15px;border:2px solid red;margin:10px;background:#fff">';
    echo '<h3 style="color:red;margin-top:0">ERROR:</h3>';
