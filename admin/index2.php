    ...
    if ($module === 'dashboard') {
                ...
                <div class="modal-dialog modal-dialog-centered"><div class="modal-content">
                    <form method="post" action="<?= e(url('admin/actions.php')) ?>" enctype="multipart/form-data">
                        <div class="modal-header">
                            <div><h5 class="modal-title" id="importSchoolsModalLabel">Import Data Sekolah</h5><small class="text-muted">Gunakan template agar kolom sesuai.</small></div>
            ...
            <div class="modal fade" id="deleteSchoolsModal" tabindex="-1" aria-labelledby="deleteSchoolsModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-lg modal-dialog-scrollable"><div class="modal-content">
                    <form method="post" action="<?= e(url('admin/actions.php')) ?>">
                        <div class="modal-header">
                            <div><h5 class="modal-title" id="deleteSchoolsModalLabel">Pilih Data Sekolah yang Dihapus</h5><small class="text-muted">Centang hanya data yang ingin dihapus.</small></div>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Tutup"></button>
                        </div>
                        <div class="modal-body">
                            <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
                ...
                <div class="row g-3 align-items-end">
                    <div class="col-md-4">
                        ...
                        <select class="form-select" name="bulan">
                            ...
                            <option value="annual" <?= $filterMonth === 0 ? 'selected' : '' ?>>Tahunan</option>
                            <?php foreach ($months as $number => $name): ?><option value="<?= $number ?>" <?= $filterMonth === $number ? 'selected' : '' ?>><?= e($name) ?></option><?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-md-auto"><button class="btn btn-pgri"><i class="fa-solid fa-filter me-2"></i>Filter</button></div>
                    <?php if ($filterMonth !== ''): ?><div class="col-md-auto"><a class="btn btn-outline-secondary" href="<?= e(url('admin/?module=finance')) ?>">Reset</a></div><?php endif; ?>
                </div>
            </form>
        ...
        <div class="card card-official"><div class="table-responsive"><table class="table mb-0 align-middle" id="memberRegistrationTable"><thead><tr><th>Pendaftar</th><th>Tempat Tugas</th><th>Kontak</th><th>Status</th><th>Catatan Admin</th><th>Aksi</th></tr></thead><tbody>
            ...
            <tr>
                ...
                <td><form method="post" action="<?= e(url('admin/actions.php')) ?>" class="registration-status-form"><input type="hidden" name="csrf_token" value="<?= csrf_token() ?>"><input type="hidden" name="module" value="member_registrations"><input type="hidden" name="id" value="<?= e($row['id']) ?>"><select class="form-select form-select-sm mb-2" name="status"><?php foreach ($statusLabels as $value => $label): ?><option value="<?= e($value) ?>" <?= $row['status'] === $value ? 'selected' : '' ?>><?= e($label) ?></option><?php endforeach; ?></select><textarea class="form-control form-control-sm mb-2" name="notes" rows="2" placeholder="Catatan verifikasi"><?= e($row['notes']) ?></textarea><button class="btn btn-sm btn-pgri">Simpan</button></form></td>
        ...
        if ($action === 'form') {
            ...
            if ($id) { $stmt = db()->prepare('SELECT * FROM documents WHERE id = ?'); $stmt->execute([$id]); $row = $stmt->fetch() ?: $row; }
            ...
            <form class="card card-official p-4" method="post" action="<?= e(url('admin/actions.php')) ?>" enctype="multipart/form-data">
                <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>"><input type="hidden" name="module" value="documents"><input type="hidden" name="id" value="<?= e((string)$row['id']) ?>">
                <div class="row g-3">
                    ...
                    <div class="col-12"><label class="form-label">File <?= !$id ? '<span class="text-danger">*</span>' : '' ?></label>
                        <?php if ($id && $row['file_path']): ?>
                            ...
                            <input class="form-control" type="file" name="file_document"><small class="text-muted">Kosongkan jika tidak mengganti file. Maksimal 100MB.</small>
                        <?php else: ?>
                            <input class="form-control" type="file" name="file_document" required><small class="text-muted">Maksimal 100MB.</small>
                        <?php endif; ?>
                    </div>
                </div>
            ...
            <?php else: ?>
            <div class="card card-official p-5 text-center"><i class="fa-solid fa-folder-open fa-4x text-muted mb-3"></i><h5>Belum ada dokumen</h5><p class="text-muted mb-0">Upload dokumen seperti PDF, Word, Excel, atau file lainnya.</p><a class="btn btn-pgri mt-3" href="<?= e(url('admin/?module=documents&action=form')) ?>"><i class="fa-solid fa-upload me-1"></i>Upload Dokumen Pertama</a></div>
            <?php endif; ?>
        <?php
        ...
        <form class="card card-official p-4" method="post" action="<?= e(url('admin/actions.php')) ?>" enctype="multipart/form-data">
            ...
            <?php foreach ($rows as $row): ?>
                ...
                <div class="mb-3"><label class="form-label">Banner Header</label><?php if ($row['setting_value']): ?><img class="img-fluid rounded mb-2" src="<?= e(media_url($row['setting_value'])) ?>" alt="Banner Header"><?php endif; ?><input type="hidden" name="settings[hero_banner]" value="<?= e($row['setting_value']) ?>"><input class="form-control" type="file" name="hero_banner" accept="image/*"><small class="text-muted">Kosongkan jika tidak mengganti banner.</small></div>
                <?php elseif ($row['setting_key'] === 'site_logo'): ?>
                <div class="mb-3"><label class="form-label">Logo Website</label><?php if ($row['setting_value']): ?><div class="mb-2"><span class="logo-mark"><img src="<?= e(media_url($row['setting_value'])) ?>" alt="Logo Website"></span></div><?php endif; ?><input type="hidden" name="settings[site_logo]" value="<?= e($row['setting_value']) ?>"><input class="form-control" type="file" name="site_logo" accept="image/*"><small class="text-muted">Kosongkan jika tidak mengganti logo.</small></div>
                <?php elseif ($row['setting_key'] === 'whatsapp_number'): ?>
                <div class="mb-3"><label class="form-label">Nomor WhatsApp Admin</label><input class="form-control" name="settings[whatsapp_number]" value="<?= e($row['setting_value']) ?>" placeholder="Contoh: 6281234567890"><small class="text-muted">Gunakan format internasional tanpa tanda plus, misalnya 6281234567890.</small></div>
                <?php else: ?>
